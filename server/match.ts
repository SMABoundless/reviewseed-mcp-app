...
// treated as a free-text keyword and checked as a substring of the title or
// abstract, since a pooled keyword's `[tiab]`/`title:`/etc. field tag means
// it was searched as free text, not necessarily present in structured
// metadata fields.
export function matchedVia(article: Article, terms: string[]): string[] {
  ...
  return terms.filter(term => {
    ...
    return mesh.has(t) || eric.has(t) || keywords.has(t) || haystack.includes(t);
  });
}
