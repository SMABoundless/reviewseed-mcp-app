...
`npm test` typechecks, then runs the unit + integration suites (398 tests, ~1.7s, no network).
...
| `unit/drift-parity.test.ts` | **Cross-repo** — MeSH history-note parsing and terminology-drift verdicts | none |
| `unit/variant-parity.test.ts` | **Cross-repo** — the tuning ladder's rungs and its skip list | none |
| `unit/crosswalk-parity.test.ts` | **Cross-repo** — MeSH↔ERIC candidates and their confidence tiers | none |
...
| `crosswalk-parity.json` | thesaurus fragment → tiered candidates | **Behavior.** The fragments are real, and two cases exist because the strongest available signal is conceptually WRONG there: MeSH `Mindfulness` matches ERIC `Metacognition`, and `Asthma` matches `Diseases`, both by Use-For synonym. The fixtures pin that neither is ever rated `strong`. |
| `variant-parity.json` | pool → rung strings + skip list | **Behavior.** Rungs are generated by the real query builder, so the fixtures also pin *which rungs are impossible and why* — a rung omitted silently makes an incomplete ladder read as a complete comparison. |
| `drift-parity.json` | real NLM history notes → clauses + verdicts | **Behavior.** Every note is copied from the live SPARQL endpoint, because the format is far less regular than documentation implies: two-digit years, `for A use B` forms, multi-clause notes, and notes whose leading year contradicts `dateIntroduced`. A test also pins that we never query the phantom `dateCreated`/`previousIndexing` predicates — that failure is silent. |
